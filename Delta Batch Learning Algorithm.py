...
#Number of  EPOCH
Inp5=2  # Dummy Dont consider for the exam
...
print('Before Updating|','weight before update|','After Updating|','the new weight|')
while Inp6 != 't':
    wT_batch_sum=np.zeros((1,no_features+1))
...
#        print(wT,xT[i].T,y)
        wT_batch_sum=wT_batch_sum+ ((tT[0][i]-y)*xT[i])
        
    wTnew= wT +  learning_rate*wT_batch_sum
    wT_before_update=wT
    wT= wTnew
