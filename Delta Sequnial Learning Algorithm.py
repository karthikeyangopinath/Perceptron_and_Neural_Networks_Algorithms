...
# #The feature Vector / Inout Array Vector
# Inp=[0,0,1,0,2,1,0,1,1,2]
# xT=np.array(Inp)
# xT=xT.reshape(5,2)
...
y_rounded=[]
wTnew_rounded=[]
#for ep in range(Inp5):
#    print('\n____________EPOCH',ep+1,'_____________________','\n') 

print('Before Updating|','weight before update|','(t - H(yx))|',' x|','  ','After Updating|','the new weight|')
while Inp6 != 't':   
    for i in range(tT_rows*tT_cols):
        ...
        wTnew= wT +  learning_rate*(tT[0][i]-y)*xT[i]
        wT_before_update=wT
        wT= wTnew
        print('Before Updating',wT_before_update,(tT[0][i]-y),xT[i],'  ','After Updating',wTnew)
